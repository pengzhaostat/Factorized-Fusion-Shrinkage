...
source('change_point_simu_function.R')
...
for (n in c(50,100,200)){
  for (prob in c(0.99,0.95,0.9)){
    
    score = NULL
    
    score_option1 =foreach (iter =1:50, .combine='c') %dopar% {
      simu_change_point(n = n, k0=2, prob =prob)
    }
    
    cat('1,\n')
    
    score_option2 =foreach (iter =1:50, .combine='c') %dopar% {
      simu_change_point(n = n, k0=5, prob =prob)
    }
    ...
    score_option3 =foreach (iter =1:50, .combine='c') %dopar% {
      simu_change_point(n = n, k0=10, prob =prob)
    }
    ...
    simu_name = paste(paste(paste("score",n,sep = ""),prob,sep = ""),'.csv',sep = "")
