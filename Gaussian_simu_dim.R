...
d = 5   # true dimension for the latent vectors
...
MF_list =  MP_Gaussain_weighted_adaptive (Y=Y, d=d0, gap =1e-4, max_iter=200,global_prior='Cauthy')
...
SMF_errors[[as.character(d0)]] <- sqrt(sum(Mix_rmse_sm))
...
SMF_errors_df <- data.frame(matrix(ncol = length(d_values), nrow = 0))
colnames(SMF_errors_df) <- as.character(d_values)
...
boxplot(SMF_errors_df,  main = "Used latent dim 2-10",
        xlab = "Used latent dimension", ylab = "RMSE Errors",
        col = "lightblue", border = "black", outline = FALSE)

boxplot(SMF_errors_df[4:9],  main = "Used latent dim 5-10",
        xlab = "Used latent dimension", ylab = "RMSE Errors",
        col = "lightblue", border = "black", outline = FALSE)
