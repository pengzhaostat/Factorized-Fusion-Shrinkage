...
MP_gibbs_mult_Sigma =  function(Y, init_sigma_X1=NULL, Sigma_list, tau =1, max_iter, init_Mean = NULL, init_Sigma = NULL,
  ...
  err[1] = sqrt(sum((X_Mean-Y)^2/T))
  
  
  err_e = 0
  if (!is.null(X_true)){
    err_e = rep(0,K)
    err_e[1] = sqrt(sum((X_Mean-X_true)^2/T))
  }
  ...
  while(k<K && ind ==0){ 
    res <- try({
      ...
      if (!is.null(X_true)){
        err_e[k] = sqrt(sum((X_Mean-X_true)^2/T))
      }
  ...
  X_list= list(Mean=X_Mean_old, Sigma = X_Sigma_old , 
               lambda= 1/sqrt(lambda), v=1/sqrt(v),tau=1/sqrt(tau),err= err,iter=k, 
               sigma_X1 = sigma_X1 , err_e =err_e)
  return(X_list)
}