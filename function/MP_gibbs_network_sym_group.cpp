...
Rcpp::List MP_gibbs_mult_Sigma(const mat& Y,  
                               const cube& Sigma_list ,double tau =1, const  arma::vec& lambda=arma::vec(),  int max_iter=200, 
                               double gap = 1e-3) {
...
Rcpp::List MP_binary_weighted_adaptive(const Cube<double>& Y,
  ...
  int T = Y.n_slices;
  int n = Y.n_rows;
  mat Y_est(n*(n-1)/2,T, fill::zeros);
...
//  Mean_X *= 0.1;
  ...
  while (k < K && ind == 0) {
    ...
    for (unsigned int i0 = 0; i0 < subset_indices.n_elem; i0++) {
      ...
      Rcpp::List MF_gibbs =  MP_gibbs_mult_Sigma (M_X_i.t(),  V_X_i, tau=tau_X, lambda = lambda, 1, inter_gap);
 ...
 //     tau_X = Rcpp::as<double>(MF_gibbs["tau"]);
      ...
      Sigma_X_new[i] = V_X;
      EX += Rcpp::as<arma::vec>(MF_gibbs["EX"]);
//      lambda(i) = Rcpp::as<arma::vec>(MF_gibbs["lambda"]);

// Rcpp::Rcout << "RcppEX " << Rcpp::as<arma::vec>(MF_gibbs["EX"]) << std::endl;
    ...
    for (int t = 0; t < (T - 1); ++t) {
      v_X(t) = 1 / (1 + lambda(t));
      b += EX(t) * lambda(t);
      lambda(t) = (n*d + 1) / (2 * (v_X(t) + EX(t) * tau / 2));
    }
    
    // Rcpp::Rcout << "b = " << b << std::endl;
    
    // Rcpp::Rcout << "EX2 = " << EX(2) << std::endl;
    
    double v0 =0;
    v0 = 1/(1+tau);
    
    tau = (n*d*(T-1)/2+1/2)/(v0+b/2);
  ...
  for(int t=0; t<T; t++){
    ...
    for( int i=0; i<n; i++){
      for( int j=0; j<n; j++){
        if(j < i){
          auc_mean(r) = 1/(1+exp( -mean_beta_new -arma::dot(Mean_X_new.slice(t).row(i), Mean_X_new.slice(t).row(j)) ));
          rmse_iter(r) = 1/(1+exp( -mean_beta -arma::dot( Mean_X.slice(t).row(i), Mean_X.slice(t).row(j) ) ));
          
          Y_est(r0,t) = auc_mean(r);
          r = r + 1;
          r0= r0+1;
        }
      }
